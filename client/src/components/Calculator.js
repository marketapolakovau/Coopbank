          ...
          <Container className={styles.slidersContainer}>
            <Row>
              <Form.Group className={styles.amountSliderGroup}>
                <Row>
                  <Form.Label className={styles.loanAmountText}>
                    {" "}
                    Chci si půjčit{" "}
                  </Form.Label>
                  <div className={styles.loanAmount}>
                    {amountSliderValue.toLocaleString()} Kč
                  </div>
                </Row>
                <Form.Range
                ...
                <Container className={styles.loanAmountRange}>
                  <Row>
                    <Col className={styles.minAmount}>
                      {minAmount.toLocaleString()} Kč
                    </Col>
                    <Col className={styles.maxAmount}>
                      {maxAmount.toLocaleString()} Kč
                    </Col>
                  </Row>
                </Container>
              </Form.Group>
            ...
            <Row>
              <Form.Group className={styles.periodSliderGroup}>
                <Row>
                  <Form.Label className={styles.loanPeriodText}>
                    Doba splácení{" "}
                  </Form.Label>

                  {monthsSliderValue === minMonths ? (
                    <div className={styles.loanPeriod}>
                      {periods?.months} {periods?.msgMonths}
                    </div>
                  ) : monthsSliderValue % 12 === 0 ? (
                    <div className={styles.loanPeriod}>
                      {periods?.years} {periods?.msgYears}
                    </div>
                  ) : (
                    <div className={styles.loanPeriod}>
                      {periods?.years || ""} {periods?.msgYears}{" "}
              ...
              <tbody>
                <tr className={styles.tableRow1}>
                  ...
                  <td
                    ...
                    id={styles.tableMonthPayment}
                  >
                    {results.monthlyPayment?.toLocaleString()} Kč
                  </td>
                </tr>
                <tr>
                  <td>Roční úroková sazba:</td>
                  <td className={styles.tableValues}>
                    {results.yearlyInterest?.toFixed(1)} %
                  </td>
                </tr>
                <tr>
                  <td>RPSN:</td>
                  <td className={styles.tableValues}>
                    {results.RPSN?.toFixed(1)} %
                  </td>
                </tr>
                <tr>
                  <td>Celková splatná částka:</td>
                  <td className={styles.tableValues}>
                    {results.overallAmount?.toLocaleString()} Kč
                  </td>
                </tr>
                {amountSliderValue > 200000 ? (
                  <tr>
                    ...
                    <td className={styles.tableValues}>
                      {results.fixedFee?.toLocaleString()} Kč
                    </td>
                  </tr>
                ) : (
                  <tr>
